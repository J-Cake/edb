...
import buffer from '@j-cake/jcake-utils/buffer';
import {Iter, iter, IterSync, iterSync} from '@j-cake/jcake-utils/iter';
...
export type Selector = (keyof any)[];
export type TypedSelector<T, Cache extends Array<PropertyKey> = []> =
    T extends PropertyKey ? Cache : {
        [P in keyof T]: [...Cache, P] | TypedSelector<T[P], [...Cache, P]>
    }[keyof T];

export type Value<Obj, selector extends TypedSelector<Obj>> = selector extends [infer key] ?
    (key extends keyof Obj ? Obj[key] : never) :
    (selector extends [infer key, ...infer subkey] ?
        (key extends keyof Obj ? (
            subkey extends TypedSelector<Obj[key]> ?
            Value<Obj[key], subkey> :
            [key] extends TypedSelector<Obj> ? Obj[key] : never) :
            never) : never);
...
export default class DB<Database> {
    ...
    private readonly changes: Map<TypedSelector<Database>, Buffer>;
    ...
    /**
     * Check whether the database contains a given key.
     **/
    public has(selector: Selector, where?: (i: string) => any): boolean {
    ...
    /**
     * Get a list of keys defined in under a given selector, as a flat list of paths, relative to `selector`
     **/
    public keys_flat(selector: Selector): string[] {
        if (!this.file)
            throw `Database not loaded`;
    ...
    /**
     * Get a list of keys defined under a given selector, in tree format, relative to `selector`
     * TODO: Rewrite
     **/
    public keys(selector: Selector): KeyList {
        const keys = this.keys_flat(selector);

        const obj: KeyList = {};
        for (const i of IterSync(keys).map(i => i.split('.')))
    ...
    /**
     * Asynchronously get a list of blocks belonging to a given selector.
     **/
    public async *get<Out>(selector: Selector): AsyncGenerator<Out> {
    ...
    /**
     * Get the value at a selector
     **/
    public async getAll<Out>(selector: Selector): Promise<Out> {
        const _selector = [...selector].map(i => (i as string).toString());
        ...
        for await (const i of this.get(selector as any))
            obj = i;
    ...
    /**
     * Get the raw buffer pointed to by `selector`
     **/
    public async *getRaw(selector: Selector, options: { maxChunkSize?: number, start?: number, end?: number }): AsyncGenerator<Buffer> {
        const _selector = [...selector].map(i => (i as string).toString());
    ...
    /**
     * Write a value into a selector
     **/
    public async set<In>(selector: Selector, value: In): Promise<void> {
        if (!this.file)
            throw `Database not loaded`;
        ...
        const setValue = async function <selector extends TypedSelector<Database>>(this: DB<Database>, selector: selector, value: Value<Database, selector>): Promise<void> {
            if (!Array.isArray(selector))
                throw `Invalid selector: ${[...selector].join('.')}`;
        ...
        const toFlatPathList = function (value: In): { [path in string]: any } {
        ...
        if (parsers.getTypeOf(value) !== null)
            return await setValue(selector as any, value as any);

        for (const [path, val] of Object.entries(toFlatPathList(value)))
            await setValue(path.split('.') as TypedSelector<Database>, val as any);
    }
     ...
     */
    public async setNext<In>(selector: Selector, value: In): Promise<void> {
     ...
     */
    public async find<Out>(selector: Selector, predicate: (i: any, a: string) => boolean): Promise<any> {
        if (!this.file)
            throw `Database not loaded`;
        ...
        const path = [...selector].filter(notNull).join('.');
        ...
        for (const i of entries) {
            const value = await this.getAll([...selector, i] as any);
            if (predicate(value, [...selector, i] as any))
                return value;
        }
    ...
    /**
     * Create the value `selector` if it does not already exist, but leave it blank
     **/
    public async define(...selectors: Selector[]): Promise<void> {
        if (!this.file)
            throw `Database not loaded`;
     ...
     * **Warning, EDB is not a journaling database. This operation is not recoverable**
     **/
    public async drop(...selectors: Selector[]): Promise<void> {
        if (!this.file)
            throw `Database not loaded`;
    ...
    private async resize(minHeaderSize: number): Promise<void> {
         ...
         * Alternatively, you could optmimise by saying splice operations less than a certain threshold should be doable in memory, but this is only feasible for small databases.                            
        */
    }
    ...
    private async shrinkRegion(selector: TypedSelector<Database>, size: number): Promise<[start: number, end: number][]> {
